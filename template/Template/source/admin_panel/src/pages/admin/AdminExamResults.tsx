...
import { PrintableExamReport } from '../../components/admin/PrintableExamReport';
...
export const AdminExamResults = () => {
    ...
    useEffect(() => {
        const fetchResults = async () => {
            ...
            // 1b. Get Student Profile
            if (sessionData.user_id) {
                const { data: profileData } = await supabase
                    .from('user_profiles')
                    .select('full_name, email, residency_year, institution')
    ...
    if (loading) return <AdminLayout title="Resultados"><div className="p-8 text-center">Calculando resultados...</div></AdminLayout>;
    if (!session) return <AdminLayout title="Error"><div className="p-8 text-center">No se encontró la sesión.</div></AdminLayout>;
    ...
    return (
        <>
        <AdminLayout title="Resultados del Examen">
            <div className="max-w-5xl mx-auto space-y-8">
                ...
                <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
                    <div className="p-4 md:p-8 text-center">
                        ...
                        <div className="flex flex-col md:flex-row justify-center gap-3 md:gap-4">
                            <Link 
                                to="/admin/students" 
                                className="flex items-center justify-center px-6 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors no-print"
                            >
                                <Home size={18} className="mr-2" /> Volver a Alumnos
                            </Link>
                            <button
                                onClick={() => window.print()}
                                className="flex items-center justify-center px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors shadow-sm no-print"
                            >
                    ...
                    <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
                        ...
                        <div className="space-y-3">
                            {breakdown.topicScores.slice(0, 5).map((t: any, idx: number) => {
                                const isExcellent = t.score >= 90;
                                const isGood = t.score >= 80;
                                const bg = isExcellent ? 'bg-green-50 border-green-100' : isGood ? 'bg-blue-50 border-blue-100' : 'bg-red-50 border-red-100';
                                const textColor = isExcellent ? 'text-green-600' : isGood ? 'text-blue-600' : 'text-red-600';
                                return (
                                    <div key={idx} className={`flex justify-between items-center p-3 rounded-lg border ${bg}`}>
                                        <div>
                                            <p className="font-medium text-gray-900 text-sm">{t.name}</p>
                                            <p className="text-xs text-gray-500">{t.count} preguntas</p>
                                        </div>
                                        <div className={`font-bold ${textColor}`}>{t.score}%</div>
                                    </div>
                                );
                            })}
                ...
                {/* Printable Report - rendered outside layout so @media print CSS can find it */}
        ...
        <PrintableExamReport
            ...
            session={session}
            answers={answers}
            islandScores={breakdown.islandScores}
