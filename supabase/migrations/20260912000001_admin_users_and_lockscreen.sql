-- ═══════════════════════════════════════════════════════════════════════════════
-- NeuroSAFEMX: Sincronización de Usuarios, Estatus de Admisión y Comité Editorial
-- ═══════════════════════════════════════════════════════════════════════════════

-- 0. Garantizar columnas necesarias en public.profiles
...
-- 1. Asegurar que TODO usuario en auth.users tenga registro en public.profiles
...
SELECT
  ...
  CASE
    WHEN lower(au.email) = 'jmyocupicior@gmail.com' THEN 'approved'::public.enrollment_status
    ELSE 'pending'::public.enrollment_status
  END,
  au.created_at,
  now()
FROM auth.users au
ON CONFLICT (id) DO UPDATE SET
  enrollment_status = CASE
    WHEN profiles.enrollment_status = 'none' AND lower((SELECT email FROM auth.users WHERE id = profiles.id)) = 'jmyocupicior@gmail.com' THEN 'approved'::public.enrollment_status
    WHEN profiles.enrollment_status = 'none' THEN 'pending'::public.enrollment_status
    ELSE profiles.enrollment_status
  END;

-- 2. Asegurar que usuarios con estatus 'none' pasen a 'pending' para que el administrador pueda revisarlos
UPDATE public.profiles
SET enrollment_status = 'pending'
WHERE enrollment_status = 'none'
  AND id != 'ae3bc0a9-bfee-4991-8f6c-56dbed3aa6fd'::uuid;

-- 3. Actualizar trigger handle_new_user() para que TODO nuevo médico quede con 'pending'
...
BEGIN
  ...
  IF v_is_superadmin THEN
    -- SuperAdmin Único (Aprobación inmediata)
    INSERT INTO public.profiles (
      id,
      display_name,
      credentials,
  ...
  ELSE
    -- Registro de Estudiante / Médico (Estado PENDING con pantalla de candado en espera de admisión)
    INSERT INTO public.profiles (
      id,
      display_name,
      credentials,
    ...
    -- Rol de estudiante para su expediente, pero su acceso al curso está sujeto a enrollment_status = 'approved'
...
-- 4. Actualizar admin_list_profiles para retornar email de auth.users y soportar todos los filtros
...
AS $$
  ...
  WHERE
    ...
    AND (
      ...
      OR (enrollment_filter = 'comite' AND EXISTS (
