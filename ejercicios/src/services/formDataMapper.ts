...
// ✨ NUEVO: Importar mappers seguros y tipos unificados
import { NullSafeMapperFactory, SafeExtractionResult } from './nullSafeMappers';
import type { ExtractedFileData, MappingResult } from '../types/enhancedSystem';

// ========== USANDO TIPOS UNIFICADOS ==========
// ExtractedFileData y MappingResult ahora vienen de types/enhancedSystem.ts
...
export class FormDataMapper {
  ...
  // ========================================
  // 1. MAPEAR SÍNTOMAS CLÍNICOS (MEJORADO CON NULL-SAFE MAPPERS)
  // ========================================
  static mapSymptomsData(extractedData: ExtractedFileData): MappingResult<ClinicalSymptomsData> {
    formLogger.log('info', 'SymptomsMapper', '🔄 Iniciando mapeo de síntomas con mappers seguros', { 
      hasSymptoms: !!extractedData.symptoms,
      symptomsKeys: extractedData.symptoms ? Object.keys(extractedData.symptoms) : []
    });

    // 🚀 USAR EL NUEVO SISTEMA DE MAPPERS SEGUROS (PACIENTE)
    const patientMapper = NullSafeMapperFactory.createPatientMapper({
      allowPartialData: true,
      strictValidation: false
    });

    // Extraer datos usando el mapper seguro como base
    const mappingResult = patientMapper.extractPatientData(JSON.stringify(extractedData));

    // Como no tenemos mapper específico para síntomas aún, usar validación segura
    // pero mantener lógica de mapeo de síntomas tradicional mejorada
    const mapperWarnings: string[] = [...mappingResult.warnings];
    const mapperErrors: string[] = [...mappingResult.errors];
    
    // Usar confianza del mapper como base
    let mapperFieldsProcessed = mappingResult.metadata.fieldsProcessed;
    let mapperFieldsMapped = mappingResult.metadata.fieldsExtracted;

    // ==== CÓDIGO LEGACY (mantener como fallback) ====
    ...
    // 🚀 COMBINAR RESULTADOS DEL MAPPER SEGURO CON LÓGICA TRADICIONAL
    const combinedWarnings = [...mapperWarnings, ...warnings];
    const combinedErrors = [...mapperErrors, ...errors];
    const totalFieldsProcessed = mapperFieldsProcessed + fieldsProcessed;
    const totalFieldsMapped = mapperFieldsMapped + fieldsMapped;
    const combinedConfidence = totalFieldsProcessed > 0 
      ? (totalFieldsMapped / totalFieldsProcessed) * 100 
      : 0;

    formLogger.log('info', 'SymptomsMapper', '✅ Mapeo híbrido completado', {
      mapperResults: { processed: mapperFieldsProcessed, mapped: mapperFieldsMapped },
      legacyResults: { processed: fieldsProcessed, mapped: fieldsMapped },
      totalFields: totalFieldsMapped,
    ...
    return {
      ...
      mappingStats: {
        ...
        fieldsSkipped: totalFieldsProcessed - totalFieldsMapped,
